...
Mat3 = Material(UniformPigment(red), SpecularBRDF(UniformPigment(red), 0.5))
Mat4 = Material(UniformPigment(cyan), DiffusiveBRDF(UniformPigment(gray), 0.8))
...
path = PathTracer(world, gray, pcg, 1, 5, 2)
#flat = Flat(world)
@btime ImgTr(path)
